 ...
 * - Circumference fields (chest, waist, hips) require a unit when value is provided
 */
export function validateMeasurementEntry(input: MeasurementInput): ValidationResult {
  ...
  if (hasHips && !input.hips_unit) {
    errors.push('Hips requires a unit (in or cm)');
  }
